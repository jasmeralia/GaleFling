Analyze the newest relevant GaleFling crash log and form or refine hypotheses about the active issue.

## Steps

1. Read `CLAUDE.md` for the configured log paths (WSL: `/mnt/c/Users/storm/AppData/Roaming/GaleFling/logs/`).
2. List the log directory and select the newest relevant crash log file. Prefer crash logs over general app logs.
3. Read only that single file. Do not scan additional logs unless there is a strong reason.
4. Analyze the crash content in the context of any active debugging session.
5. Return:
...
- Prefer crash logs over app logs for this command.
