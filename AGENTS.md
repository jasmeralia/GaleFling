...
All build artifacts and release tags derive from this one string automatically — do not create version strings anywhere else in the codebase. When bumping manually, change `APP_VERSION`, `README.md`, and `CHANGELOG.md`; build metadata is generated.
...
| GitHub release tag | Created by `.github/workflows/release.yml` from `APP_VERSION` |

**Rule: never hard-code a version string in any file other than `src/utils/constants.py`, `README.md`, and `CHANGELOG.md`.**

On `master` pushes, the release workflow reads `APP_VERSION`. If that version is not newer than the latest release tag, the workflow commits the next patch version to `src/utils/constants.py`, `README.md`, and `CHANGELOG.md`, then lets the follow-up workflow run tag and build that exact code version.
