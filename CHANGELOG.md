...
- **FetLife post URLs were never captured:** `SUCCESS_URL_PATTERN` expected `users/<id>/statuses/<id>`, but FetLife status permalinks are `fetlife.com/<username>/s/<id>`. No text post could ever match, so URL capture silently never fired. Both forms are now accepted.
