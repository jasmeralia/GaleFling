...
- **Fansly functional coverage:** connection/session tests, a composer-elements test, a mutating text post, and a mutating **image post** that drives Fansly's own upload flow and proves the published post contains the attachment. `_attach_media()` and `_media_attachment_state()` moved from `FetLifePlatform` to `BaseWebViewPlatform` so both platforms share one implementation instead of a copy.
