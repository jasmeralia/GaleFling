...
- **Media can be attached through Chromium's file picker:** `BaseWebViewPlatform` gained `stage_media_for_picker()` / `open_media_picker()`, and `_LoggingWebEnginePage` now overrides `chooseFiles()` to satisfy a page's picker from the staged selection. The page receives a genuine `change` event with a real `File`, which is what site uploaders expect. Note this is **not** sufficient for Fansly: the picker fires and Chromium hands over a real selection, but Fansly's composer ignores it as thoroughly as it ignores a synthetic `DataTransfer`, so Fansly still has no media coverage. Chromium refuses to open a picker without user activation, so a trusted gesture must precede it; in the application that is the user's own click. With nothing staged the override falls through to the native dialog, so opening a picker by hand still behaves normally.
- **Fansly functional coverage:** connection/session tests, a composer-elements test, and a mutating text post that proves the post reaches the feed. Media attach is deliberately **not** covered — Fansly's uploader ignores a synthetic file selection entirely (see below), so any such test would be untestable rather than merely unwritten. `_attach_media()` and `_media_attachment_state()` moved from `FetLifePlatform` to `BaseWebViewPlatform` so both platforms share one implementation instead of a copy.
