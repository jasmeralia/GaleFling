...
`test_text_post_creates_a_post` posts a real status and does not delete it. It reports
this rather than claiming success: the run prints `CLEANUP PENDING` with the tag. Delete
it from your feed after a mutating run.

This is deliberate, not a limitation: it matches every other mutating WebView test in
this suite (Fansly, and FetLife's own picture/video tests), which leave what they create
up unconditionally for manual inspection — see `functional_cleanup.py`'s module
docstring and `AGENTS.md` rule 9. A working deletion mechanism exists
(`_delete_status_by_tag()` in `test_webview_fetlife.py`, using a trusted
`QTest`-style mouse click since FetLife's Delete handler is bound in JavaScript and
ignores a synthetic `.click()`) but is not called by any test — it is retained for task
#420's planned opt-in deletion pass, which will decide how and when to invoke it.
