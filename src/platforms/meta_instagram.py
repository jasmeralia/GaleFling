...
class MetaInstagramPlatform(BasePlatform):
    ...
    def _validate_pre_post(self, text: str, media_paths: list[Path] | None) -> str | None:
        ...
        if specs.max_text_length is not None and len(text) > specs.max_text_length:
            get_logger().warning(
                f'Instagram pre-post validation failed: text too long '
            ...
            for path in media_paths:
                ...
                if suffix in _VIDEO_EXTENSIONS:
                    ...
                    if fmt not in specs.supported_video_formats:
                        get_logger().warning(
                            f'Instagram pre-post validation failed: '
            ...
            if expires_at:
                try:
                    ...
                    if expiry <= datetime.now(UTC):
                        get_logger().warning(
                            f'Instagram pre-post validation failed: token expired at {expires_at}'
                        )
    ...
    def _stage_media(self, file_path: Path) -> str:
        ...
        get_logger().debug(f'Instagram S3 staging: uploading {file_path.name} ({size_mb:.2f} MB)')
        try:
            url = stager.upload_media(file_path)
            get_logger().debug(f'Instagram S3 staging complete: {file_path.name}')
            return url
    ...
    def _create_media_container(
        ...
        *,
        ...
        get_logger().debug(f'Instagram media container created: {container_id}')
        return container_id
    ...
    def _create_carousel_container(self, caption: str, item_ids: list[str]) -> str:
        ...
        self._raise_for_status(resp)
        container_id = resp.json()['id']
        get_logger().debug(f'Instagram carousel container created: {container_id}')
        return container_id
    ...
    def _wait_for_container(self, container_id: str) -> None:
        ...
        """
        get_logger().debug(f'Instagram polling container {container_id} (timeout={_POLL_TIMEOUT}s)')
        attempt = 0
        ...
        while time.monotonic() < deadline:
            ...
            get_logger().debug(
                f'Instagram container {container_id} poll #{attempt}: status={status!r}'
            )
            if status == 'FINISHED':
                get_logger().debug(
                    f'Instagram container {container_id} finished after {attempt} poll(s)'
                )
                return
            if status == 'ERROR':
